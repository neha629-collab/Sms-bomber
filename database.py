...
def populate_initial_apis(cur):
    """ডাটাবেস খালি থাকলে ডিফল্ট API-গুলো যোগ করে।"""
    initial_apis = [
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS users (
                        user_id BIGINT PRIMARY KEY, first_name VARCHAR(255),
                        username VARCHAR(255), join_date TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
                    );
                """)
                cur.execute("""
