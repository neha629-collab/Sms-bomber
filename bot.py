...
(MAIN_MENU, ADMIN_PANEL, ASK_NUMBER, ASK_AMOUNT, BROADCAST_CONFIRM, ASK_USER_ID, 
 MANAGE_ADMINS, ADD_ADMIN_ID, REMOVE_ADMIN_ID, MANAGE_BANS, BAN_USER_ID, UNBAN_USER_ID) = range(12)
...
admin_keyboard = [
    ...
    ["👥 Manage Admins", "🚫 Manage Bans"], # নতুন বাটন
    ["🌡️ API Status", "⬅️ Back to Main Menu"]
]
...
# --- নতুন ব্যান ম্যানেজমেন্ট কিবোর্ড ---
manage_bans_keyboard = [
    ["🚫 Ban User", "✅ Unban User"],
    ["📋 List Banned", "⬅️ Back to Admin Panel"]
]
manage_bans_markup = ReplyKeyboardMarkup(manage_bans_keyboard, resize_keyboard=True)
...
# --- Main Handlers (ব্যান চেকসহ আপডেটেড) ---
...
# --- Bombing Conversation (ব্যান চেকসহ আপডেটেড) ---
async def ask_for_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return ASK_NUMBER
# ... (ask_for_amount এবং start_bombing অপরিবর্তিত) ...
...
# ... (Broadcast, User Stats, API Status ফিচার অপরিবর্তিত) ...
...
# ... (add_admin, remove_admin, list_admins ফাংশন অপরিবর্তিত) ...
...
# --- নতুন ব্যান ম্যানেজমেন্ট ফিচার ---
async def manage_bans_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text("Manage user bans:", reply_markup=manage_bans_markup)
    return MANAGE_BANS

async def ban_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text("Please send the User ID to ban.")
    return BAN_USER_ID
...
async def unban_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text("Please send the User ID to unban.")
...
async def list_banned(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    message = "<b>🚫 Banned Users:</b>\n"
    for user_id in banned_ids:
        try:
            user_chat = await context.bot.get_chat(user_id)
            message += f"• {user_chat.first_name} (@{user_chat.username}) - <code>{user_id}</code>\n"
        except Exception:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ADMIN_PANEL: [
                ...
                MessageHandler(filters.Regex('^🚫 Manage Bans$'), manage_bans_start), # নতুন handler
            ...
            # --- নতুন state এবং handler ---
            MANAGE_BANS: [
                MessageHandler(filters.Regex('^🚫 Ban User$'), ban_user_start),
                MessageHandler(filters.Regex('^✅ Unban User$'), unban_user_start),
                MessageHandler(filters.Regex('^📋 List Banned$'), list_banned),
                MessageHandler(filters.Regex('^⬅️ Back to Admin Panel$'), admin_panel_handler),
            ],
            BAN_USER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, ban_user_finish)],
            UNBAN_USER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, unban_user_finish)],
        },
